# GenoFLU — OOD sandbox app (Slurm, no admin)

Per-user deployment of GenoFLU on any Open OnDemand HPC. The card appears
...
bin/bdtools install --sandbox genoflu_gui        # builds env+frontend, links this card
$EDITOR ~/.../ood/apps/genoflu_gui_sandbox/form.yml   # set cluster: "CHANGE_ME" -> your cluster
```

`bdtools install --sandbox genoflu_gui` writes `~/.config/genoflu_gui/sandbox.env`
(`BDTOOLS_APP_DIR` + `BDTOOLS_APP_ENV`) and symlinks this card into
`~/ondemand/dev/genoflu_gui`. The launcher (`template/script.sh.erb`) sources
