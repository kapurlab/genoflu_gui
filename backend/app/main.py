...
def _rp_metrics(run_dir: Path) -> Dict[str, Any]:
    """Genotype and how many segments contributed to the call."""
    d = _rp_json(run_dir, "genoflu_result.json")
    used = d.get("list_used") or []
    return {
        "genotype": d.get("genotype") or None,
        "segments": (len(used) or None),
    }
...
_RP_EXPORT_COLUMNS = [("sample", "Sample"), ("status", "Status"),
                      ...
                      ("qc_reasons", "QC notes")] + [("genotype", "Genotype"), ("segments", "Segments")] + \
...
def api_project_results_csv(name: str, start: str = Query(""), end: str = Query(""),
    ...
    return Response(content=buf.getvalue(), media_type="text/csv",
                    headers={"Content-Disposition":
                             'attachment; filename="%s_genoflu_results.csv"' % name})
...
def api_project_results_xlsx(name: str, start: str = Query(""), end: str = Query(""),
    ...
    return Response(content=buf.getvalue(),
                    ...
                    headers={"Content-Disposition":
                             'attachment; filename="%s_genoflu_results.xlsx"' % name})
